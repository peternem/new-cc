...
<div class="">
    <section class="content-area container-fluid white">
                ...
                <tbody>
                    ...
                    <?php //print_r($pc);?>
                            <tr>
                                <td class="title">
                                    <?php
                                    if (has_post_thumbnail($pc->ID)) {
                                        echo '<a href="' . get_permalink($pc->ID) . '" title="' . esc_attr($pc->post_title) . '" class="artwork-link">';
                                        echo get_the_post_thumbnail($pc->ID, 'thumbnail', array( 'class' => 'artwork'  ));
                                        echo '</a>';
                                    } else {
                                        echo '<a href="' . get_permalink($pc->ID) . '" title="' . esc_attr($pc->post_title) . '" class="artwork-link">';
                                        echo "<img src=\"/wp-content/uploads/2015/11/wood-bground-1-2500x1406-150x150.jpg\" class=\"artwork\"/>";
                                        echo "</a>";
                                    }
                                    ?>
                                    <div class="title-content"><a role="link" title="<?php echo $pc->post_title; ?>" href="/<?php echo $pc->post_name; ?>" class="perma-link"><?php echo $pc->post_title; ?></a>
                                    <?php 
                                    if($pc->post_excerpt) {
                                        echo "<p class=\"truncate\">".$pc->post_excerpt."</p>";
                                    }
                                    ?>
                                    </div>
                                    </li>
                                ...
                                <td class="date">
                                    <div class="listDate">
                                        ...
                                        if (get_field('event_start_date')) {
                                            ...
                                            $cleanStartDate = date("m/d/y", strtotime($startDate));
                                            echo "<em>" . $cleanStartDate . "</em>";

                                            if ((!$endUtc == $startUtc) || ($endUtc > $startUtc)) {
                                                if ((!$endUtc < $startUtc)) {
                                                    $cleanEndDate = date("m/d/y", strtotime($endDate));
        ...
        if (get_field('event_start_time')) {
            echo "<em>" . get_field('event_start_time', $pc->ID) . "</em>";
        }
        if (get_field('event_end_time')) {
            echo " - ";
            echo "<em>" . get_field('event_end_time', $pc->ID) . "</em>";
        }
        ?>
                                        <?php
                                        ?></div>
